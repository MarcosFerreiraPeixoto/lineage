...
class SafeMock:
    def __init__(self, name="", root=False):
        ...
        self._is_root = root
        self.terminal = False
        self.caller_name = "unknown"  # Default caller name

    def __getattr__(self, attr):
        if attr.startswith('__') and attr.endswith('__'):
            return super().__getattr__(attr)
        new_name = f"{self.name}.{attr}" if self.name else attr
        return SafeMock(name=new_name)
    
    def __call__(self, *args, **kwargs):
        if not self.terminal:
            self.call_args_list.append((self.caller_name, args, kwargs))
            self.terminal = True
        new_name = f"{self.name}.result" if self.name else "result"
        return SafeMock(name=new_name)
...
def create_mock_namespace():
    ...
    def create_and_register_mock(func_name):
        def wrapper(*args, **kwargs):
            ...
            new_mock.caller_name = caller_name  # Set the caller name
        ...
        try:
            ...
            for func_name, mock_instance in mock_registry:
                if func_name in function_names:
                    for caller_name, call_args, call_kwargs in mock_instance.call_args_list:
                        processed_args = [
                        ...
                        results.append({
                            'function': func_name,
                            'caller': caller_name,  # Add caller information
                            'args': processed_args,
